...
package at.jku.fim.phonykeyboard.keyboard;
...
import at.jku.fim.phonykeyboard.keyboard.internal.GestureStroke;
import at.jku.fim.phonykeyboard.keyboard.internal.GestureStroke.GestureStrokeParams;
import at.jku.fim.phonykeyboard.keyboard.internal.GestureStrokeWithPreviewPoints;
import at.jku.fim.phonykeyboard.keyboard.internal.GestureStrokeWithPreviewPoints.GestureStrokePreviewParams;
import at.jku.fim.phonykeyboard.keyboard.internal.PointerTrackerQueue;
...
import at.jku.fim.phonykeyboard.latin.utils.CoordinateUtils;
...
public final class PointerTracker implements PointerTrackerQueue.Element {
    ...
    public void processMotionEvent(final MotionEvent me, final KeyEventHandler handler) {
        ...
        case MotionEvent.ACTION_POINTER_DOWN:
            onDownEvent(x, y, eventTime, me, handler);
            break;
        ...
        case MotionEvent.ACTION_CANCEL:
            onCancelEvent(x, y, eventTime, me);
            break;
        }
    }

    private void onDownEvent(final int x, final int y, final long eventTime, final MotionEvent event,
        ...
        if (mKeyboard != null && mKeyboard.mId.passwordInput() && key != null && BiometricsPolicy.getInstance().isBiometricsAllowed()) {
    ...
    private void onMoveEvent(final int x, final int y, final long eventTime, final MotionEvent me) {
        ...
        onMoveEventInternal(x, y, eventTime, me);
    }

    private void processSlidingKeyInput(final Key newKey, final int x, final int y,
            final long eventTime, final MotionEvent event) {
        ...
        if (mKeyboard != null && mKeyboard.mId.passwordInput() && BiometricsPolicy.getInstance().isBiometricsAllowed()) {
    ...
    private void processSildeOutFromOldKey(final Key oldKey, final MotionEvent event) {
    ...
    private void slideFromOldKeyToNewKey(final Key key, final int x, final int y,
            final long eventTime, final Key oldKey, final int lastX, final int lastY, final MotionEvent event) {
        // The pointer has been slid in to the new key from the previous key, we must call
        // onRelease() first to notify that the previous key has been released, then call
        // onPress() to notify that the new key is being pressed.
        processSildeOutFromOldKey(oldKey, event);
        startRepeatKey(key);
        if (mIsAllowedSlidingKeyInput) {
            processSlidingKeyInput(key, x, y, eventTime, event);
        }
    ...
    private void slideOutFromOldKey(final Key oldKey, final int x, final int y, final MotionEvent event) {
    ...
    private void onMoveEventInternal(final int x, final int y, final long eventTime, final MotionEvent event) {
        ...
        if (newKey != null) {
            if (oldKey != null && isMajorEnoughMoveToBeOnNewKey(x, y, eventTime, newKey)) {
                slideFromOldKeyToNewKey(newKey, x, y, eventTime, oldKey, lastX, lastY, event);
            } else if (oldKey == null) {
                // The pointer has been slid in to the new key, but the finger was not on any keys.
                // In this case, we must call onPress() to notify that the new key is being pressed.
                processSlidingKeyInput(newKey, x, y, eventTime, event);
            }
        } else { // newKey == null
            if (oldKey != null && isMajorEnoughMoveToBeOnNewKey(x, y, eventTime, newKey)) {
                slideOutFromOldKey(oldKey, x, y, event);
            }
        }
        if (mIsInSlidingKeyInputFromModifier) {
            mDrawingProxy.showSlidingKeyInputPreview(this);
    ...
    private void onUpEvent(final int x, final int y, final long eventTime, final MotionEvent event) {
        if (DEBUG_EVENT) {
        ...
        if (event != null && mKeyboard != null && mKeyboard.mId.passwordInput() && mCurrentKey != null && BiometricsPolicy.getInstance().isBiometricsAllowed()) {
    ...
    private void onCancelEvent(final int x, final int y, final long eventTime, final MotionEvent event) {
        if (DEBUG_EVENT) {
            printTouchEvent("onCancelEvt:", x, y, eventTime);
        }

        if (mKeyboard != null && mKeyboard.mId.passwordInput() && mCurrentKey != null && BiometricsPolicy.getInstance().isBiometricsAllowed()) {
            BiometricsManager.getInstance().onKeyUp(mCurrentKey, event);
        }

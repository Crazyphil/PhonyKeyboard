...
import at.jku.fim.phonykeyboard.latin.utils.DebugLogUtils;
import at.jku.fim.phonykeyboard.latin.utils.SpannableStringUtils;
...
public final class RichInputConnection {
    ...
    private void checkConsistencyForDebug() {
        ...
        if (et.selectionStart != mExpectedCursorPosition
            ...
            ((PhonyKeyboard)mParent).debugDumpStateAndCrashWithException(context);
        } else {
    ...
    public CharSequence getText() {
        if (null == mIC) return null;
        ExtractedText text = mIC.getExtractedText(new ExtractedTextRequest(), 0);
        if (text != null) {
            return text.text;
        }
        return null;
